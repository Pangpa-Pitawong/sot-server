...
async function saveRoom(room) {
  try {
    await window.storage.set(storeKey(room.code), JSON.stringify(room), true);
    return true;
  } catch { return false; }
}
...
async function loadRoom(code) {
  try {
    const r = await window.storage.get(storeKey(code), true);
    return r ? JSON.parse(r.value) : null;
  } catch { return null; }
}

async function deleteRoom(code) {
  try { await window.storage.delete(storeKey(code), true); } catch {}
}
  ...
  try {
    ...
    for (const key of res.keys) {
      try {
        ...
        if (r) rooms.push(JSON.parse(r.value));
      } catch {}
    }
    return rooms.filter(r => r && r.status !== "started" && (Date.now() - r.createdAt) < 30*60*1000);
  } catch { return []; }
}
...
function assignRoles(n) {
  const roles = ["king"];
  const rc = n >= 5 ? 2 : 1;
  for (let i=0; i<rc; i++) roles.push("rebel");
...
export default function App() {
  ...
  // ─── BROWSE ROOMS ────────────────────────────────────────────────────────
  const browseRooms = useCallback(async () => {
  ...
  const createRoom = async () => {
    ...
    setLoading(false);
    if (!ok) { showToast("❌ สร้างห้องไม่สำเร็จ ลองใหม่"); return; }
    setMyName(name);
    setMyIdx(0);
    setMyClass("");
    setRoom(newRoom);
  ...
  // ─── JOIN ROOM ───────────────────────────────────────────────────────────
  const joinRoom = async (codeArg, nameArg) => {
